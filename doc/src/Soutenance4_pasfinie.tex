...
\'etions fix\'e. On peut remarquer, que presque tous 
ont \'et\'e respect\'e (sachant qu'une personne a abandonn\'e en cours
de route).
...
\dessin{structure.eps}{0.4}{Le squelette du projet}{structure}
...
\dessin{ville.epsf}{0.9}{Mod\`ele d'un bloc  de la ville}{bloc}
...
\dessin{suspension.ps}{0.4}{Une Audi Monster}{monster}
...
l'action $\delta \AAA$ associ\'ee a la une variation de la trajectoire
...
avec  $x_1(t)$ et $x_2(t)$ les positions \`a l'instant des deux masses.
...
On obtient les m\^emes \'equations que celles que l'on on obtiendrait
en appliquant la loi fondamentale de la dynamique $m\vec{\gamma} =
\vec{F}$.

\subsection{Monocycle}\label{dyna2}
...
Comme dans la section pr\'ec\'edente, on trouve un syst\`eme
d'\'equation diff\'erentielle o\`u les inconnues sont l'altitude de la
roue et de la carcasse :
\begin{align}
...
L'\'energie potentielle due \`a la pesanteur des deux roues est : $$mg(2y+y_2+y_1)\;.$$
 
L'\'energie potentielle du ressort de la roue avant est : $$1/2ky_1^2\;.$$


L'\'energie potentielle du ressort de la roue arri\`ere est : $$1/2ky_2^2\;.$$
...
On a :
\begin{align}
(M+2m)\ddot{y}+m\ddot{y}_1+m\ddot{y}_2 & = -g(2m+M)+R_1+R_2\label{eqy}\;,\\
...
L'int\'egration de ces \'equations donne le mouvement horizontal des voitures.

\section{La circulation}\label{circulation}

\dessin{feu.ps}{0.4}{La pomme est mure}{feurouge}
...
\dessin{feux.eps}{0.5}{Plan de feux}{feux}
...
On voit deux types de mots cl\'es :  -- ceux qui utilisent des champs et
-- ceux qui ont une accolade entrante. Les premiers sont soit des sommets,
soit des triangles, soit des informations, -- les autres constituent des
listes.
...
L'utilisation de liste chain\'ee est plus astucieuse que celle de
...
la transparence de l'objet. Pour plus d'informations sur les lumi\`eres conf\'ere section \ref{Lumieres}.
...
\dessin{texture.epsf}{0.8}{Coordonn\'ees d'un mat\'eriau sur un objet carr\'e}{texture}
...
Voici ce que nous voulons obtenir sous OpenGl (un seul triangle est repr\'esent\'e)~:
\begin{verbatim}
...
\end{verbatim}
Une texture est d\'efinie par un entier (un {\tt Gluint}) lors de l'initialisation.
Chaque {\tt glTexCoord3f} indique les sommets de la texture. 
...
Une cam\'era est d\'efinie par une position $p$ dans l'espace
...
modifie le sens d l'image (par exemple, on voit diff\'erament selon
notre environnement si on a la t\^ete \`a l'envers).  Dans notre
exemple, nous avons : $p=(100.0,100.0,100.0)$, $c=(1.0,1.0,1.0)$ et
$o=(0.0,1.0,0.0)$.

\section{Effets sp\'eciaux}
\subsection{Les liste d'affichage d'OGL} 
...
\subsection{Les lumi\`eres}\label{Lumieres}
...
\subsubsection{Des polygones qui d\'eg\`enerent}
...
par Tampon.  Le nombre r\'eel d'enregistrements entiers lus (inférieur
ou \'egal à Compte) est renvoy\'e dans Transfere.  On utilisera aussi
...
\subsubsection{La cr\'eation du terrain}
...
\dessin{terr.epsf}{0.8}{Maillage avec les faces transversales}{grille}
...
\dessin{bez.epsf}{0.8}{Courbe de Bezier}{Bezier}
...
consid\`ere tous les objets de la sc\`ene. Il est donc dans ce cas utile de
refaire \`a la main la projection au lieu de laisser faire OpenGL. On
d\'efinit les objets que l'on veut projeter et on fait la projection
apr\`es avoir retrouv\'e les param\`etres de la projection utilis\'es
par OpenGL qui sont les deux matrices : {\tt GL\_PROJECTION\_MATRIX,
GL\_MODELVIEW\_MATRIX}.
\subsection{Menu Delphi}

\dessin{menu.ps}{0.4}{Le menu d'Ecstasy}{menu}

\subsection{Sons et video}
\subsection{Installation}
\section{Web et credits}

www.epita.fr$\backslash\!\!\sim$quadra\_q

%\zigzag{.8}{.9}

%\begin{tabular}{|r|c|c|c|c|}
%\multicolumn{1}{r}{}
%& \rotatebox{45}{Soutenance 1}
%& \rotatebox{45}{Soutenance 1}
%& \rotatebox{45}{Soutenance 1}
%& \rotatebox{45}{Soutenance 1}\\
%& \multicolumn{1}{c}{\makebox[0cm][l]{\rotatebox{45}{Soutenance 1}}}
%& \multicolumn{1}{c}{\makebox[0cm][l]{\rotatebox{45}{Soutenance 2}}}
%& \multicolumn{1}{c}{\makebox[0cm][l]{\rotatebox{45}{Soutenance 3}}}
%& \multicolumn{1}{c}{\makebox[0cm][l]{\rotatebox{45}{Soutenance 4}}}\\
%\hline
%ghghgh & 10 & 20 & 30 & 50 \\
%\end{tabular}
\begin{thebibliography}{50}
%\bibitem{GTA}
\bibitem{STARDUST} http://www.soton.ac.uk/~trawww/stardust
